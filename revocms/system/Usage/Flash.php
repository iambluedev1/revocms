...
class Flash {
    ...
    public function __construct() {
        ...
        if(@is_null($rcms->themes->selected)){
            die("Please select a default template");
        }
        foreach ($this->msgWrapper as $wrapper => $value){
            $this->msgWrapper[$wrapper] = $rcms->themes->selected->flash->$wrapper->template;
        }
        foreach ($this->msgTypes as $type => $value){
            $this->msgTypes[$type] = $rcms->themes->selected->flash->class->$type;
        }
